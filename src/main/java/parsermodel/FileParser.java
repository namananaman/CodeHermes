...
public class FileParser {
	...
	public static ParserResult parseFile(String file) {
		...
		ArrayList<Integer> startMatchings = getMatchings(START_COMMENT_TOKEN,
				file, true);
		...
		ArrayList<Integer> startLineNumbers = getCorrespondingLineNumbers(
				startMatchings, lineNumbers);
		ArrayList<Integer> endLineNumbers = getCorrespondingLineNumbers(
				endMatchings, lineNumbers);
		...
		for (int i = 0; i < size; i++) {
			...
			String comment = removeAsteriskAndTabs(file.substring(startLine,
					endLine).trim());
	...
	private static ParserResult getInlineComments(ParserResult currentResult,
			String file) {
		ArrayList<String> comments = new ArrayList<String>();
		...
		for (int i = 0; i < size; i++) {
			inlineCommentsToLineNumbers
					.put(comments.get(i), lineNumbers.get(i));
		}
